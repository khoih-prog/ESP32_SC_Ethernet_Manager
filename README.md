...
* [Why do we need this ESP32_SC_Ethernet_Manager library](#why-do-we-need-this-ESP32_SC_Ethernet_Manager-library)
  * [Features](#features)
...
### Why do we need this [ESP32_SC_Ethernet_Manager library](https://github.com/khoih-prog/ESP32_SC_Ethernet_Manager)

#### Features

This is an `ESP32_S2/S3/C3 + LwIP W5500` Credentials and Connection Manager with fallback Web ConfigPortal. This Library is used for configuring ESP32 Ethernet Static / DHCP and Credentials at runtime. You can specify static DNS servers, personalized HostName and CORS feature.
...
This [**ESP32_SC_Ethernet_Manager** library](https://github.com/khoih-prog/ESP32_SC_Ethernet_Manager) currently supports these following boards:

 1. **ESP32_S3 boards** using `LwIP W5500 or ENC28J60 Ethernet`
 

#### ESP32S3_DEV

<p align="center">
    <img src="https://github.com/khoih-prog/ESP32_SC_Ethernet_manager/raw/main/Images/ESP32S3_DEV.png">
</p> 
...
<p align="center">
    <img src="https://github.com/khoih-prog/ESP32_SC_Ethernet_Manager/raw/main/Images/W5500.png">
</p>

<p align="center">
    <img src="https://github.com/khoih-prog/ESP32_SC_Ethernet_Manager/raw/main/Images/W5500_small.png">
</p> 
...
<p align="center">
    <img src="https://github.com/khoih-prog/ESP32_SC_Ethernet_Manager/raw/main/Images/ENC28J60.png">
</p>
...
## Prerequisites
 ...
 5. [`WebServer_ESP32_SC_ENC library v1.0.0+`](https://github.com/khoih-prog/WebServer_ESP32_SC_ENC) if necessary to use `ESP32_S2/S3/C3` boards using LwIP ENC28J60 Ethernet. To install, check [![arduino-library-badge](https://www.ardu-badge.com/badge/WebServer_ESP32_SC_ENC.svg?)](https://www.ardu-badge.com/WebServer_ESP32_SC_ENC)
...
The best and easiest way is to use `Arduino Library Manager`. Search for `ESP32_SC_Ethernet_Manager`, then select / install the latest version. You can also use this link [![arduino-library-badge](https://www.ardu-badge.com/badge/ESP32_SC_Ethernet_Manager.svg?)](https://www.ardu-badge.com/ESP32_SC_Ethernet_Manager) for more detailed instructions.

### Manual Install

1. Navigate to [ESP32_SC_Ethernet_Manager](https://github.com/khoih-prog/ESP32_SC_Ethernet_Manager) page.
2. Download the latest release `ESP32_SC_Ethernet_Manager-main.zip`.
3. Extract the zip file to `ESP32_SC_Ethernet_Manager-main` directory 
4. Copy the whole `ESP32_SC_Ethernet_Manager-main` folder to Arduino libraries' directory such as `~/Arduino/libraries/`.

### VS Code & PlatformIO:

1. Install [VS Code](https://code.visualstudio.com/)
2. Install [PlatformIO](https://platformio.org/platformio-ide)
3. Install [**ESP32_SC_Ethernet_Manager** library](https://registry.platformio.org/libraries/khoih-prog/ESP32_SC_Ethernet_Manager) by using [Library Manager](https://registry.platformio.org/libraries/khoih-prog/ESP32_SC_Ethernet_Manager/installation). Search for **ESP32_SC_Ethernet_Manager** in [Platform.io Author's Libraries](https://platformio.org/lib/search?query=author:%22Khoi%20Hoang%22)
...
Thanks to [Roshan](https://github.com/solroshan) to report the issue in [Error esp_littlefs.c 'utime_p'](https://github.com/khoih-prog/ESP32_SC_Ethernet_Manager/issues/28) 
...
```cpp
#include <ESP32_SC_Ethernet_Manager.hpp>               //https://github.com/khoih-prog/ESP32_SC_Ethernet_Manager
```

in many files. But be sure to use the following `#include <ESP32_SC_Ethernet_Manager.h>` **in just 1 `.h`, `.cpp` or `.ino` file**, which must **not be included in any other file**, to avoid `Multiple Definitions` Linker Error
...
```cpp
//ESP32_SC_Ethernet_manager.setSTAStaticIPConfig(stationIP, gatewayIP, netMask, dns1IP, dns2IP);
ESP32_SC_Ethernet_manager.setSTAStaticIPConfig(WM_STA_IPconfig);
```
...
```cpp
String tempTZ = ESP32_SC_Ethernet_manager.getTimezoneName();
```
...
```cpp
const char * TZ_Result = ESP32_SC_Ethernet_manager.getTZ(_timezoneName);
```
...
```cpp
ESP32_SC_Ethernet_Manager ESP32_SC_Ethernet_manager("Personalized-HostName");
```

then later call

```cpp
ESP32_SC_Ethernet_manager.startConfigPortal()
```
...
These illustrating steps is based on the example [ConfigOnSwitchFS](https://github.com/khoih-prog/ESP32_SC_Ethernet_Manager/tree/main/examples/ConfigOnSwitchFS)
...
The example [ConfigOnSwitchFS](https://github.com/khoih-prog/ESP32_SC_Ethernet_Manager/tree/main/examples/ConfigOnSwitchFS) will open the CP whenever a SW press is detected in `loop()`. So the code to add `dynamic variables` will be there, just after the CP `ESP32_SC_Ethernet_Manager` class initialization to create `ESP32_SC_Ethernet_Manager` object.
...
Adding those `ESP32_EMParameter` objects created in Step 2 using the function `addParameter()` of object `ESP32_SC_Ethernet_Manager`
...
ESP32_SC_Ethernet_manager.addParameter(&p_thingspeakApiKey);
ESP32_SC_Ethernet_manager.addParameter(&p_sensorDht22);
ESP32_SC_Ethernet_manager.addParameter(&p_pinSda);
ESP32_SC_Ethernet_manager.addParameter(&p_pinScl);
```
...
After users select `Save`, the CP `ESP32_SC_Ethernet_Manager` object will save the user input data into related `ESP32_EMParameter` objects.
...
<p align="center">
    <img src="https://github.com/khoih-prog/ESP32_SC_Ethernet_Manager/raw/main/Images/Saved.png">
</p>
...
```cpp
ESP32_SC_Ethernet_manager.setSaveConfigCallback(saveConfigCallback);
```
...
void loop()
{
  ...
  if ((digitalRead(TRIGGER_PIN) == LOW) || (digitalRead(TRIGGER_PIN2) == LOW))
  {
    ...
    ESP32_SC_Ethernet_Manager ESP32_SC_Ethernet_manager("ConfigOnSwitch");
    ...
    ESP32_SC_Ethernet_manager.setSTAStaticIPConfig(stationIP, gatewayIP, netMask);
#endif
#endif
    ...
    else
    {
      ...
      ESP32_SC_Ethernet_manager.setConfigPortalTimeout(0);
    ...
    if (!ESP32_SC_Ethernet_manager.startConfigPortal())
    ...
    const char * TZ_Result = ESP32_SC_Ethernet_manager.getTZ(Ethconfig.TZ_Name);
    ...
    ESP32_SC_Ethernet_manager.getSTAStaticIPConfig(EthSTA_IPconfig);
...
Many applications need configuration parameters like `MQTT host and port`, [Blynk](http://www.blynk.cc) or [emoncms](http://emoncms.org) tokens, etc. While it is possible to use [`ESP32_SC_Ethernet_Manager`](https://github.com/khoih-prog/ESP32_SC_Ethernet_Manager) to collect additional parameters, it is better to read these parameters from a web service once [`ESP32_SC_Ethernet_Manager`](https://github.com/khoih-prog/ESP32_SC_Ethernet_Manager) has been used to connect to the Internet.

To capture other parameters with [`ESP32_SC_Ethernet_Manager`](https://github.com/khoih-prog/ESP32_SC_Ethernet_Manager) is a little bit more complicated than all the other features. This requires adding custom HTML to your form. 

If you want to do it with [`ESP32_SC_Ethernet_Manager`](https://github.com/khoih-prog/ESP32_SC_Ethernet_Manager) see the example [ConfigOnSwitchFS](examples/ConfigOnSwitchFS)
...
```cpp
ESP32_SC_Ethernet_manager.setSTAStaticIPConfig(IPAddress(192,168,2,232), IPAddress(192,168,2,1), IPAddress(255,255,255,0));
```
...
```cpp
ESP32_SC_Ethernet_manager.setCustomHeadElement("<style>html{filter: invert(100%); -webkit-filter: invert(100%);}</style>");
```
...
ESP32_SC_Ethernet_manager.addParameter(&custom_text);
```
...
https://github.com/khoih-prog/ESP32_SC_Ethernet_Manager/blob/c5b9c0c67a9df6347e0d22e61c27c9876780ea38/examples/ConfigOnSwitch/ConfigOnSwitch.ino#L25-L1025
...
ESP32_SC_Ethernet_Manager v1.0.0
...
Local Date/Time: Fri Dec 16 00:35:19 2022
Stop doubleResetDetecting
Saving config file...
Saving config file OK
Local Date/Time: Fri Dec 16 00:36:19 2022
Local Date/Time: Fri Dec 16 00:37:19 2022
Local Date/Time: Fri Dec 16 00:38:19 2022
```
...
Config file was successfully saved
[EM] freeing allocated params!
Local Date/Time: Fri Dec 16 01:08:57 2022
Local Date/Time: Fri Dec 16 01:09:57 2022
```
...
```cpp
ESP32_SC_Ethernet_manager.setDebugOutput(false);
```
...
Submit issues to: [ESP32_SC_Ethernet_Manager issues](https://github.com/khoih-prog/ESP32_SC_Ethernet_Manager/issues)
...
### DONE
 ...
 1. Add support to **ESP32S3-based boards** using `LwIP W5500 or ENC28J60 Ethernet`
...
- The library is licensed under [MIT](https://github.com/khoih-prog/ESP32_SC_Ethernet_Manager/blob/main/LICENSE)
